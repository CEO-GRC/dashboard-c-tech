"""QA Tab v6.0 - FIXED: Filtros y Fechas"""
...
def render_qa_tab(df_aging, col_payer=None, col_name=None, col_collector=None, col_total=None):
    """Tab Management QA v6.0 - FIXED: Filtros y Fechas."""
    ...
    with c3:
        # NUEVO: Filtro de estado mejorado
        status = st.selectbox(
            "Estado:", 
    ...
    if status == 'Solo Gestionadas':
        ...
        st.info(f"✅ Mostrando solo cuentas **gestionadas**")
    elif status == 'Solo NO Gestionadas':
        df_view = df_view[df_view['was_touched'] == False]
        st.warning(f"⚠️ Mostrando solo cuentas **SIN gestionar**")
    ...
    # Tabla con FECHA en lugar de días
    tbl = []
    for _, r in df_view.iterrows():
        ...
        # NUEVA COLUMNA: Fecha formateada
        ...
        tbl.append({
            ...
            'Company': company,
            'Balance': fmt(balance),
            'Last Activity': last_date,  # <- FECHA EN VEZ DE DÍAS
