"""
AR Collections Intelligence Dashboard — v4.0 (FINAL)
======================================================
Columnas reales del archivo SAP esperadas después de normalización:
  · 'Total'           → monto total AR
  · 'Current'         → saldo al día
  · 'Payer'           → nombre del cliente
  · 'Payer.1'         → ID numérico SAP del pagador
  · 'Collector'       → cobrador asignado
  · 'Region'          → región geográfica
  · 'Cr/Mgt Risk Category' → categoría de riesgo
  · '1 - 30 days'     → bucket 1-30 días (después de reemplazar \n por ' ')
  · '31 - 60 days'
  · '61 - 90 days'
  · '91 - 120 days'
  · '121 - 180 days'
...
T = {
    'EN': {
        ...
        'instructions':'Upload the SAP Aging Report in .xlsx format. Column names are auto-normalised.',
        'no_data':'No data loaded',
        'no_data_sub':'Upload your SAP Aging Report from the left panel to start the portfolio analysis.',
        'err_cols':'Could not find the "Total" or "Current" columns. Please verify the file format.',
        'no_filter_results':'No accounts match the current filters. Adjust your selection.',
        ...
        'payer_name':'Payer Name', 'payer_id':'Payer ID', 'risk_cat':'Risk Category',
        ...
        'info_footer':'{n} accounts · Overdue: {total} ({pct}% of portfolio) · Avg: {avg} · 90+d critical: {crit} ({pct90}%)',
    },
    'ES': {
        'title':'Collections', 'title_acc':'Dashboard',
        ...
        'instructions':'Sube el Reporte de Antigüedad SAP en .xlsx. Los nombres de columna se normalizan automáticamente.',
        'no_data':'Sin datos cargados',
        'no_data_sub':'Sube tu Reporte de Antigüedad SAP desde el panel izquierdo.',
        'err_cols':'No se encontraron las columnas "Total" o "Current". Verifica el formato del archivo.',
        'no_filter_results':'Ninguna cuenta coincide con los filtros actuales.',
        ...
        'pd_by_risk':'Vencido por Categoría de Riesgo', 'pd_by_tier':'Vencido por Nivel',
        ...
        'payer_name':'Nombre Pagador', 'payer_id':'ID Pagador', 'risk_cat':'Categoría Riesgo',
        ...
        'info_footer':'{n} cuentas · Vencido: {total} ({pct}% del portafolio) · Prom: {avg} · 90+d: {crit} ({pct90}%)',
    }
}
t = T[LANG]
...
html,body,[class*="css"],.stApp {{
    ...
    background:{BG0} !important; color:{T1} !important;
}}
...
[data-testid="collapsedControl"] {{
    background:{BG2} !important; border:1px solid {BORDER} !important; color:{T1} !important;
}}
...
.ts-chip {{
    ...
    color:{T2};background:{BG2};border:1px solid {BORDER};border-radius:6px;
    padding:5px 12px;white-space:nowrap;
}}
.empty-state {{
    margin:5rem auto;max-width:400px;background:{BG1};
    border:1px solid {BORDER};border-radius:16px;padding:3rem 2.5rem;text-align:center;
}}
...
[data-testid="stDataFrame"] thead th {{
    ...
    font-size:.57rem !important;letter-spacing:.1em !important;text-transform:uppercase !important;
    font-family:'JetBrains Mono',monospace !important;font-weight:700 !important;
}}
[data-testid="stDataFrame"] tbody td {{
    font-family:'JetBrains Mono',monospace !important;
    font-size:.75rem !important;color:{T1} !important;background:{BG1} !important;
}}
.stDownloadButton>button {{
    background:linear-gradient(135deg,#ff6b35,#d94e1f) !important;
    color:#fff !important;border:none !important;border-radius:8px !important;
    font-size:.72rem !important;font-weight:700 !important;
    letter-spacing:.05em !important;padding:.5rem 1.4rem !important;
}}
...
[data-testid="stTabs"] [data-baseweb="tab-list"] {{
    background:{BG1};border-radius:8px;border:1px solid {BORDER};padding:4px;gap:2px;
}}
...
def clean_money(series: pd.Series) -> pd.Series:
    """
    Ultra-robust numeric cleaner.
    Accepts: numeric Series, string Series with $, commas, spaces, NaN, empty strings.
    Returns: float64 Series — every unparseable value becomes 0.0.
    """
...
def fmt(v) -> str:
    """Format a value as a dollar amount. Always returns a string, never raises."""
    try:
        return f"${float(v):,.2f}"
...
def fmtk(v) -> str:
    """Format a value as $K / $M. Always returns a string, never raises."""
    try:
        v = float(v)
    except (TypeError, ValueError):
        v = 0.0
...
def pct(part, total) -> float:
    try:
        return round(float(part) / float(total) * 100, 1) if float(total) else 0.0
...
def load_data(file_bytes: bytes) -> pd.DataFrame:
    """
    Load Excel and normalise column names:
      1. Convert every \\n inside a column name to a space.
      2. Collapse multiple spaces and strip edges.
    Result: '1 - 30\\ndays' → '1 - 30 days'
    """
...
def safe_bar_chart(x_vals, y_vals, orientation='v', colors=None,
    ...
    """
    Build a Plotly bar chart defensively.
    All values are coerced to float before rendering.
    If data is empty, returns a placeholder figure.
    """
    try:
...
try:
    ...
    st.error(f"❌ Error al leer el archivo / Error reading file: {e}")
    st.stop()


# ══════════════════════════════════════════════════════════════════════════════
# COLUMN MAPPING — STRICT, EXACT NAMES
# After load_data normalization '\ndays' → ' days', so bucket names are:
#   '1 - 30 days', '31 - 60 days', ... '> 365 days'
# ══════════════════════════════════════════════════════════════════════════════

# -- Core columns (exact names from the SAP report) --
COL_NAME  = 'Payer'     if 'Payer'     in df_raw.columns else None   # Payer Name
COL_PAYER = 'Payer.1'   if 'Payer.1'   in df_raw.columns else None   # Payer ID (numeric)
COL_TOTAL = 'Total'     if 'Total'     in df_raw.columns else None
COL_CURR  = 'Current'   if 'Current'   in df_raw.columns else None
COL_COLL  = 'Collector' if 'Collector' in df_raw.columns else None
COL_REG   = 'Region'    if 'Region'    in df_raw.columns else None
COL_LOC   = 'Location'  if 'Location'  in df_raw.columns else None
COL_TERMS = next((c for c in df_raw.columns if 'payment terms' in c.lower()), None)
COL_RISK  = next((c for c in df_raw.columns if 'cr/mgt risk' in c.lower() or
                                               'risk category' in c.lower()), None)

# -- Bucket columns — exact post-normalization names --
# These are the fixed names after load_data converts '\ndays' → ' days'
BUCKET_EXACT = [
    ('1-30d',    '1 - 30 days'),
    ('31-60d',   '31 - 60 days'),
    ('61-90d',   '61 - 90 days'),
    ('91-120d',  '91 - 120 days'),
    ('121-180d', '121 - 180 days'),
    ('181-365d', '181 - 365 days'),
    ('365+d',    '> 365 days'),
]

# BUCKETS maps friendly label → actual column name (only if column exists)
BUCKETS = {
    label: col
    for label, col in BUCKET_EXACT
    if col in df_raw.columns
}

# -- Validate required columns --
missing = [name for name, col in [('Total', COL_TOTAL), ('Current', COL_CURR)]
           if col is None]
if missing:
    st.error(f"⚠️ {t['err_cols']}")
    st.markdown(f"**Columnas detectadas:** `{', '.join(df_raw.columns.tolist())}`")
    st.dataframe(df_raw.head(3))
    st.stop()

if not BUCKETS:
    st.warning(
        "⚠️ No se detectaron columnas de bucket de antigüedad. "
        f"Se esperaban: {[col for _, col in BUCKET_EXACT]}. "
        f"Columnas actuales: {df_raw.columns.tolist()}"
    )
...
for col in [COL_TOTAL, COL_CURR] + list(BUCKETS.values()):
    if col:
        df[col] = clean_money(df[col])

# Past Due = sum of all bucket columns (robust against messy Total/Current data)
# Falls back to Total - Current only if no buckets found.
if BUCKETS:
    bucket_col_list = list(BUCKETS.values())
    df['_PD'] = df[bucket_col_list].apply(
        lambda row: row.fillna(0).clip(lower=0).sum(), axis=1
    )
else:
    df['_PD'] = (df[COL_TOTAL] - df[COL_CURR]).clip(lower=0)

# Computed per-bucket columns (prefix _B_)
for label, col in BUCKETS.items():
    df[f'_B_{label}'] = df[col].fillna(0).clip(lower=0)
...
with col_ph:
    sel_coll = st.multiselect(t['filter_coll'], _unique_sorted(COL_COLL),
                               placeholder=t['all_coll'])
with reg_ph:
    sel_reg  = st.multiselect(t['filter_reg'],  _unique_sorted(COL_REG),
                               placeholder=t['all_reg'])
with risk_ph:
    sel_risk = st.multiselect(t['filter_risk'], _unique_sorted(COL_RISK),
                               placeholder=t['all_risk'])

dff = df.copy()
if sel_coll and COL_COLL: dff = dff[dff[COL_COLL].astype(str).isin(sel_coll)]
if sel_reg  and COL_REG:  dff = dff[dff[COL_REG].astype(str).isin(sel_reg)]
if sel_risk and COL_RISK: dff = dff[dff[COL_RISK].astype(str).isin(sel_risk)]


# ── METRICS — safe defaults when dff is empty ────────────────────────────────
if dff.empty:
    total_ar = total_cur = total_pd = 0.0
    pct_pd = pct_cur = 0.0
    n_total = 0
else:
    total_ar  = float(dff[COL_TOTAL].sum())
    total_cur = float(dff[COL_CURR].sum())
    # total_pd  = sum of bucket columns (.sum().sum() pattern, NaN-safe)
    if BUCKETS:
        bkt_cols = list(BUCKETS.values())
        total_pd = float(dff[bkt_cols].fillna(0).clip(lower=0).sum().sum())
    else:
...
# Per-bucket totals (all guaranteed floats)
bkt = ({label: float(dff[f'_B_{label}'].sum()) for label in BUCKETS}
       if not dff.empty else {label: 0.0 for label in BUCKETS})

pd_90p  = sum(v for k, v in bkt.items()
...
def kpi_card(color, label, value_html, sub, bar_w, bar_color=None):
    ...
    hex_map = {'orange':ORANGE,'green':GREEN,'red':RED,'yellow':YELLOW,
               'blue':BLUE,'purple':PURPLE}
    hex_c = hex_map.get(bc, ORANGE)
    return f"""<div class='kpi {color}'>
        <div class='kpi-label'>{label}</div>
        <div class='kpi-value'>{value_html}</div>
        <div class='kpi-sub'>{sub}</div>
        <div class='kpi-bar'>
            <div class='kpi-bar-fill' style='width:{min(float(bar_w),100):.0f}%;background:{hex_c}'></div>
        </div>
    </div>"""
...
with k3:
    ...
    st.markdown(kpi_card('red', t['kpi_pd'],
        ...
        f"{n_od} {t['accts']} · {pct_pd}% {t['at_risk']}", pct_pd, 'red'),
        unsafe_allow_html=True)
...
with tab1:
    try:
        ...
        if dff.empty:
            st.info(t['no_filter_results'])
        else:
            bnames   = list(bkt.keys())
            bamounts = [float(v) for v in bkt.values()]
            b_colors = BUCKET_COLORS[:len(bnames)]

            ca, cb, cc = st.columns([4, 2, 2], gap="small")

            with ca:
                st.plotly_chart(
                    safe_bar_chart(bnames, bamounts, colors=b_colors,
                                   title=t['overdue_by_bucket']),
                    use_container_width=True)

            with cb:
                cur_v = max(float(total_cur), 0)
                pd_v  = max(float(total_pd),  0)
                if cur_v + pd_v > 0:
            ...
            with cc:
                ...
                all_c = [GREEN] + b_colors
                st.plotly_chart(
                    safe_bar_chart(all_l, all_v, orientation='h',
                                   colors=all_c, title=t['balance_by_tier']),
                    use_container_width=True)

            # Bucket composition mini-cards
            if bkt:
                ...
                for i, (bname, bval) in enumerate(bkt.items()):
                    ...
                    with comp_cols[i]:
                        st.markdown(f"""<div class='kpi' style='padding:14px 16px;text-align:center'>
                            ...
                            <div style='font-size:1.3rem;font-weight:900;color:{bc}'>{fmtk(float(bval))}</div>
...
with tab2:
    try:
        ...
        if dff.empty:
            ...
            st.warning(t['no_collector'])
        else:
            cg = dff.groupby(COL_COLL, as_index=False).agg(
...
with tab3:
    try:
        ...
        if dff.empty:
            ...
            st.warning(t['no_risk'])
        else:
            rkg = dff.groupby(COL_RISK, as_index=False).agg(
                Total_AR =(COL_TOTAL, 'sum'),
                Past_Due =('_PD',     'sum'),
                Accounts =(COL_TOTAL, 'count'),
            ).sort_values('Past_Due', ascending=False)
            ...
            high_pd = float(rkg[rkg[COL_RISK].apply(
                lambda x: 'HIGH' in str(x).upper() or '/H' in str(x).upper()
            )]['Past_Due'].sum())
            med_pd  = float(rkg[rkg[COL_RISK].apply(
                lambda x: 'MED' in str(x).upper() or '/M' in str(x).upper()
            )]['Past_Due'].sum())
            low_pd  = max(total_pd - high_pd - med_pd, 0.0)
        ...
        # Risk detail table — always shown when data exists
        if not dff.empty:
            ...
            # Build column list from only what exists
            risk_candidates = [COL_NAME, COL_PAYER, COL_RISK, COL_LOC,
                               COL_REG, COL_TERMS, COL_TOTAL, '_PD']
            risk_show = [c for c in risk_candidates if c and c in dff.columns]

            if risk_show:
                risk_df = dff[risk_show].copy().sort_values('_PD', ascending=False)

                # Rename map — built safely, only existing columns
                rrmap = {}
                if COL_NAME  and COL_NAME  in risk_df.columns: rrmap[COL_NAME]  = t['payer_name']
                if COL_PAYER and COL_PAYER in risk_df.columns: rrmap[COL_PAYER] = t['payer_id']
                if COL_RISK  and COL_RISK  in risk_df.columns: rrmap[COL_RISK]  = t['risk_cat']
                if COL_LOC   and COL_LOC   in risk_df.columns: rrmap[COL_LOC]   = t['location']
                if COL_REG   and COL_REG   in risk_df.columns: rrmap[COL_REG]   = t['region']
                if COL_TERMS and COL_TERMS in risk_df.columns: rrmap[COL_TERMS] = t['payment_terms']
                if COL_TOTAL and COL_TOTAL in risk_df.columns: rrmap[COL_TOTAL] = t['total_ar']
                if '_PD'     in risk_df.columns:              rrmap['_PD']      = t['past_due']

                risk_display  = risk_df.rename(columns=rrmap)
                risk_download = risk_display.copy()

                for mc in [t['total_ar'], t['past_due']]:
                    if mc in risk_display.columns:
                        risk_display[mc] = risk_display[mc].apply(fmt)

                btn_r, _ = st.columns([2, 5])
                with btn_r:
                    st.download_button(
                        label=t['download_risk'],
                        data=to_excel_bytes(risk_download),
...
with tab4:
    try:
        ...
        if dff.empty:
            ...
            # Label preference: Payer name → Payer ID → generic index
            ...
            # ── Full action table ──────────────────────────────────────────
            # Simplified columns: Payer.1 (ID), Payer (Name), Total, _PD, buckets
            # No optional columns that might be None — every entry is verified first.
            st.markdown(f"""<div class='sec-hdr' style='margin-top:0'>
                <span class='sec-title'>{t['full_list']}</span>
                <span class='sec-badge'>{t['dl_ready']}</span>
            </div>""", unsafe_allow_html=True)

            # Step 1 — build column list (only existing, never None)
            action_cols = []
            if COL_PAYER and COL_PAYER in df_sla.columns:
                action_cols.append(COL_PAYER)        # Payer.1 = numeric ID
            if COL_NAME  and COL_NAME  in df_sla.columns:
                action_cols.append(COL_NAME)          # Payer = company name
            if COL_COLL  and COL_COLL  in df_sla.columns:
                action_cols.append(COL_COLL)
            if COL_REG   and COL_REG   in df_sla.columns:
                action_cols.append(COL_REG)
            if COL_RISK  and COL_RISK  in df_sla.columns:
                action_cols.append(COL_RISK)
            if COL_TOTAL and COL_TOTAL in df_sla.columns:
                action_cols.append(COL_TOTAL)
            action_cols.append('_PD')                 # always present
            for label in BUCKETS:
                bc = f'_B_{label}'
                if bc in df_sla.columns:
                    action_cols.append(bc)

            action_df = df_sla[action_cols].copy()
            export_df = action_df.copy()   # raw numbers for Excel download

            # Step 2 — rename map built only from columns that exist
            # (This is the fix for the line-672 rmap NoneType crash)
            rmap = {}
            if COL_PAYER and COL_PAYER in action_df.columns:
                rmap[COL_PAYER] = t['payer_id']          # 'Payer.1' → 'Payer ID'
            if COL_NAME  and COL_NAME  in action_df.columns:
                rmap[COL_NAME]  = t['payer_name']         # 'Payer'   → 'Payer Name'
            if COL_COLL  and COL_COLL  in action_df.columns:
                rmap[COL_COLL]  = t['collector']
            if COL_REG   and COL_REG   in action_df.columns:
                rmap[COL_REG]   = t['region']
            if COL_RISK  and COL_RISK  in action_df.columns:
                rmap[COL_RISK]  = t['risk_cat']
            if COL_TOTAL and COL_TOTAL in action_df.columns:
                rmap[COL_TOTAL] = t['total_ar']
            rmap['_PD'] = t['past_due']
            for label in BUCKETS:
                bc = f'_B_{label}'
                if bc in action_df.columns:
                    rmap[bc] = label    # e.g. '_B_1-30d' → '1-30d'

            action_df = action_df.rename(columns=rmap)
            export_df = export_df.rename(columns=rmap)

            # Step 3 — format currency columns for display (not for download)
            fmt_targets = [t['total_ar'], t['past_due']] + list(BUCKETS.keys())
            for col in fmt_targets:
                if col in action_df.columns:
                    action_df[col] = action_df[col].apply(fmt)

            # Step 4 — download button (uses export_df with raw numbers)
