...
T = {
    'EN': {
        ...
        'no_filter_results':'No accounts match the current filters. Please adjust your selection.',
    ...
    'ES': {
        ...
        'no_filter_results':'Ninguna cuenta coincide con los filtros actuales. Ajusta tu selección.',
...
def clean_money(s):
    """
    Safely converts a Series to float.
    Handles: already-numeric, NaN, empty strings, and $-formatted strings.
    Always returns a float Series; unparseable values become 0.
    """
    ...
    cleaned = (
        s.astype(str)
         ...
         .replace({'': '0', 'nan': '0', 'NaN': '0', 'None': '0', '-': '0'})
    )
...
def fmtk(v):
    """Format a numeric value as $K or $. Always expects a Python float/int."""
    try:
        v = float(v)
    except (TypeError, ValueError):
        v = 0.0
...
def pct(p, total):
    return round(p / total * 100, 1) if total else 0
...
def find_col(df, patterns):
    """
    Find the first column whose name contains any of the given patterns.
    Comparison is case-insensitive and strips newlines/extra spaces.
    """
    normalized = {c: c.lower().replace('\n', ' ').strip() for c in df.columns}
    for p in patterns:
        p_low = p.lower().strip()
        for col, col_norm in normalized.items():
            if p_low in col_norm:
                return col
    return None
...
def load_data(file_bytes):
    ...
    # Normalize column names: collapse newlines + extra spaces into a single space
...
def safe_bar_chart(x_vals, y_vals, orientation='v', colors=None,
    ...
    """Build a Plotly bar chart safely — handles empty or non-numeric data."""
    # Coerce all values to float, defaulting to 0 on failure
    try:
    ...
    # Build label texts — use values appropriate to the orientation
    label_vals = y_vals if orientation == 'v' else y_vals
    texts = [fmtk(v) for v in label_vals]
...
# ══════════════════════════════════════════════════════════════════════════════
# COLUMN MAPPING — strict names matching the SAP Aging Report format
# The load_data function normalises column names (newlines → spaces).
# After normalisation the bucket columns look like: "1 - 30 days", "31 - 60 days", etc.
# ══════════════════════════════════════════════════════════════════════════════

# Core columns — exact / near-exact matches first, then fallbacks
COL_PAYER  = find_col(df_raw, ['Payer.1', 'Payer ID', 'PayerID'])   # numeric SAP payer code
COL_NAME   = find_col(df_raw, ['Name 1', 'Name1', 'Name', 'Customer', 'Client', 'Payer'])  # company name
...
# ── Bucket columns ────────────────────────────────────────────────────────────
# After load_data normalization the names become: "1 - 30 days", "31 - 60 days", etc.
# We map each friendly label → actual column name (or None if not found).
BUCKET_DEFS = [
    ('1-30d',    ['1 - 30']),
    ('31-60d',   ['31 - 60']),
    ('61-90d',   ['61 - 90']),
...
BUCKETS = {}   # label → actual column name
for label, patterns in BUCKET_DEFS:
...
if not BUCKETS:
    st.warning("⚠️ No aging bucket columns detected. Aging tabs will show $0. "
               "Expected columns: '1 - 30 days', '31 - 60 days', etc.")
...
numeric_cols = [c for c in [COL_TOTAL, COL_CURR] + list(BUCKETS.values()) if c]
for c in numeric_cols:
    df[c] = clean_money(df[c])

# Past Due = sum of all bucket columns (safer than Total - Current when data is messy)
# Falls back to Total - Current if no buckets are detected.
if BUCKETS:
    bucket_cols = list(BUCKETS.values())
    df['_PD'] = df[bucket_cols].apply(
        lambda row: row.fillna(0).clip(lower=0).sum(), axis=1
    )
else:
    df['_PD'] = (df[COL_TOTAL] - df[COL_CURR]).clip(lower=0)

# Per-bucket computed columns (prefix _B_)
for bname, bcol in BUCKETS.items():
    df[f'_B_{bname}'] = df[bcol].fillna(0).clip(lower=0)
...
# ── METRICS (all default to 0 when dff is empty) ──────────────────────────────
if dff.empty:
...
bkt = {b: float(dff[f'_B_{b}'].sum()) for b in BUCKETS} \
      if not dff.empty else {b: 0.0 for b in BUCKETS}
...
with tab1:
    ...
    if dff.empty:
        ...
        # Bucket values are already floats from bkt dict
        bnames   = list(bkt.keys())
        bamounts = [float(v) for v in bkt.values()]   # guaranteed float list
        ...
        with cc:
            ...
            all_v = [float(total_cur)] + bamounts   # all floats
            all_c = [GREEN] + colors
...
with tab2:
    ...
    # Regional breakdown (independent of collector check)
...
with tab3:
    ...
    if not dff.empty:
        ...
        risk_show_candidates = [COL_NAME, COL_PAYER, COL_RISK, COL_LOC, COL_REG,
                                COL_TERMS, COL_TOTAL, '_PD']
        risk_show = [c for c in risk_show_candidates if c and c in dff.columns]

        if risk_show:
            risk_df = dff[risk_show].copy().sort_values('_PD', ascending=False)

            # Build rename map — only non-None keys that are present in the dataframe
            rename_risk = {}
            if COL_NAME  and COL_NAME  in risk_df.columns: rename_risk[COL_NAME]  = t['customer']
            if COL_PAYER and COL_PAYER in risk_df.columns: rename_risk[COL_PAYER] = t['payer_id']
            if COL_RISK  and COL_RISK  in risk_df.columns: rename_risk[COL_RISK]  = t['risk_cat']
            if COL_LOC   and COL_LOC   in risk_df.columns: rename_risk[COL_LOC]   = t['location']
            if COL_REG   and COL_REG   in risk_df.columns: rename_risk[COL_REG]   = t['region']
            if COL_TERMS and COL_TERMS in risk_df.columns: rename_risk[COL_TERMS] = t['payment_terms']
            if COL_TOTAL and COL_TOTAL in risk_df.columns: rename_risk[COL_TOTAL] = t['total_ar']
            if '_PD'     in risk_df.columns:               rename_risk['_PD']     = t['past_due']
...
with tab4:
    ...
    if dff.empty:
        ...
        # Use Name 1 if available, fall back to Payer ID, then generic index
        ...
        # ── Full action table ──────────────────────────────────────────────
        # Simplified export: Payer ID, Name 1, Past Due total, per-bucket columns
        # Only columns that actually exist in df_sla are included.
        st.markdown(f"""<div class='sec-hdr' style='margin-top:0'>
            <span class='sec-title'>{t['full_list']}</span>
            <span class='sec-badge'>{t['dl_ready']}</span>
        </div>""", unsafe_allow_html=True)

        # Build column list strictly from what exists
        action_candidates = []
        if COL_PAYER and COL_PAYER in df_sla.columns: action_candidates.append(COL_PAYER)
        if COL_NAME  and COL_NAME  in df_sla.columns: action_candidates.append(COL_NAME)
        if COL_COLL  and COL_COLL  in df_sla.columns: action_candidates.append(COL_COLL)
        if COL_REG   and COL_REG   in df_sla.columns: action_candidates.append(COL_REG)
        if COL_RISK  and COL_RISK  in df_sla.columns: action_candidates.append(COL_RISK)
        if COL_TOTAL and COL_TOTAL in df_sla.columns: action_candidates.append(COL_TOTAL)
        action_candidates.append('_PD')   # always present
        for b in BUCKETS:
            bc = f'_B_{b}'
            if bc in df_sla.columns:
                action_candidates.append(bc)

        action_df = df_sla[action_candidates].copy()
        export_df = action_df.copy()

        # Rename map — only include keys that exist, never None
        rmap = {'_PD': t['past_due']}
        if COL_PAYER and COL_PAYER in action_df.columns: rmap[COL_PAYER] = t['payer_id']
        if COL_NAME  and COL_NAME  in action_df.columns: rmap[COL_NAME]  = t['customer']
        if COL_COLL  and COL_COLL  in action_df.columns: rmap[COL_COLL]  = t['collector']
        if COL_REG   and COL_REG   in action_df.columns: rmap[COL_REG]   = t['region']
        if COL_RISK  and COL_RISK  in action_df.columns: rmap[COL_RISK]  = t['risk_cat']
        if COL_TOTAL and COL_TOTAL in action_df.columns: rmap[COL_TOTAL] = t['total_ar']
        for b in BUCKETS:
            bc = f'_B_{b}'
            if bc in action_df.columns:
                rmap[bc] = b   # use the friendly bucket label as column header

        action_df = action_df.rename(columns=rmap)
        export_df = export_df.rename(columns=rmap)

        # Format currency columns for display only
        fmt_cols = [t['total_ar'], t['past_due']] + list(BUCKETS.keys())
        for mc in fmt_cols:
