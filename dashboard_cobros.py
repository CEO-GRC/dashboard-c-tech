...
def clean_money(s):
    ...
    Handles: already-numeric values, NaN, empty strings, and currency-formatted strings.
    Always returns a float Series with 0 replacing any unparseable value.
    """
    # If the series is already numeric, just fill NaN with 0
    if pd.api.types.is_numeric_dtype(s):
        return s.fillna(0).astype(float)
    # Otherwise, convert to string, strip currency chars, then parse
    return pd.to_numeric(
        s.astype(str)
         .str.strip()
         .replace({'': '0', 'nan': '0', 'NaN': '0', 'None': '0'})
...
# ── METRICS ───────────────────────────────────────────────────────────────────
# Guard: all metrics default to 0 when dff is empty (e.g. aggressive filters)
if dff.empty:
...
df_sla    = dff[dff['_PD'] > 0.01].copy().sort_values('_PD', ascending=False) if not dff.empty else pd.DataFrame()
n_od      = len(df_sla)
n_ok      = n_total - n_od
...
bkt = {b: float(dff[f'_B_{b}'].sum()) for b in BUCKETS} if not dff.empty else {b: 0.0 for b in BUCKETS}
...
with tab3:
    ...
    if dff.empty:
        st.info("No data available to display in the risk table.")
    else:
        ...
        # Filter out None keys and keys not present in the dataframe
        rename_risk = {k: v for k, v in rename_risk.items() if k and k in risk_df.columns}
...
with tab4:
    ...
    if df_sla.empty:
        ...
        # Filter out None keys AND keys not present in action_df to avoid KeyErrors
        rmap = {k: v for k, v in rmap.items() if k and k in action_df.columns}
