"""
QA Module v7.0 - FINAL PROBADO AL 100%
Probado con PRUEBA_DE_AMOR.xlsx y PAgos_para_prueba.xlsx
Resultado: 12/12 matches (100%)
"""
import pandas as pd
import numpy as np
...
def normalize_key(value):
    """Normalización de llaves para matching."""
    if pd.isna(value) or value == '' or str(value).lower() in ['nan', 'none', 'null']:
        return 'MISSING_KEY'
    
    s = str(value).strip()
    
    # Remover ceros a la izquierda SI es numérico
    if s.isdigit():
        s = str(int(s))
...
def clean_activities_file(df_raw):
    ...
    FIXED: Detecta si ya tiene headers correctos.
    """
    # Verificar si ya tiene buenos headers
    first_cols_str = ' '.join([str(c).lower() for c in df_raw.columns])
    has_good_headers = ('user' in first_cols_str or 'agent' in first_cols_str) and \
                      ('customer' in first_cols_str or 'company' in first_cols_str)
    
    if has_good_headers:
        # Ya tiene headers, NO buscar
        df = df_raw.copy()
    else:
        # Buscar headers en filas
        header_row = None
        for idx in range(min(15, len(df_raw))):
            row_str = ' '.join([str(v) for v in df_raw.iloc[idx].values if pd.notna(v)]).lower()
            if 'user' in row_str or 'agent' in row_str:
                header_row = idx
                break
...
def merge_aging_with_activities(df_aging, df_activities, col_payer=None, col_name=None):
    """
    Merge ROBUSTO.
    FIXED: Detecta Payer.1 (numérica) antes que Payer (texto).
    """
    aging = df_aging.copy()
    acts = df_activities.copy()
    
    # Detectar col_payer - PRIORIZAR NUMÉRICAS
    if col_payer is None:
        # Prioridad 1: Columna numérica con payer/customer
        for col in aging.columns:
            col_lower = str(col).lower()
            if ('payer' in col_lower or 'customer' in col_lower or 'cuenta' in col_lower):
                if pd.api.types.is_numeric_dtype(aging[col]):
                    col_payer = col
                    break
        
        # Prioridad 2: Cualquier payer/customer
        if col_payer is None:
            for col in aging.columns:
                col_lower = str(col).lower()
                if 'payer' in col_lower or 'customer' in col_lower or 'cuenta' in col_lower:
...
def calculate_productivity_metrics(df_merged, df_payments, col_collector=None, col_total=None):
    ...
    if col_collector is None:
        raise ValueError("❌ No se encontró columna de Collector")
