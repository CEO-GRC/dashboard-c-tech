...
def clean_activities_file(df_raw):
    ...
    """
    # Verificar si necesitamos buscar headers
    # Si la primera fila tiene columnas con nombres apropiados, no buscar
    ...
    if has_good_headers:
        # Ya tiene buenos headers, usar tal cual
        df = df_raw.copy()
    else:
...
def merge_aging_with_activities(df_aging, df_activities, col_payer=None, col_name=None):
    ...
    if col_payer is None:
        # Prioridad 1: Buscar columna numérica con payer/customer
        ...
        # Prioridad 2: Si no encontró numérica, buscar cualquiera
        if col_payer is None:
            for col in aging.columns:
                col_lower = str(col).lower()
                if 'payer' in col_lower or 'customer' in col_lower or 'cuenta' in col_lower:
    ...
    agg.columns = ['_match_key', 'agentes_consolidados', 'last_activity_date', 'total_activities', 'recent_history']
    ...
    merged['agentes_consolidados'] = merged['agentes_consolidados'].fillna('Sin Asignar')
    merged['agentes_consolidados'] = merged['agentes_consolidados'].replace({'': 'Sin Asignar', 'nan': 'Sin Asignar'})
...
def get_multi_agent_portfolio(df_merged, df_activities, agent_names, col_collector=None):
    ...
    def row_belongs_to_agent(row):
        ...
        # B) ¿Está en agentes_consolidados?
        if 'agentes_consolidados' in row.index:
            agents_str = str(row['agentes_consolidados']).strip().upper()
