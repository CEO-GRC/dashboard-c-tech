...
def fmt_days(v):
    """Formatear DÍAS como número entero."""
    try:
        num = float(v)
        if pd.isna(num) or num == 0:
            return "0"
        # Si es timestamp gigante, formatear con comas
        if num > 100000:
            return f"{int(num):,}"
        return f"{int(num)}"
    except:
...
def render_qa_tab(df_aging, col_payer=None, col_name=None, col_collector=None, col_total=None):
    """Tab Management QA v5.1 - SIN ERRORES."""
    ...
    with c1:
        ...
        st.markdown("**Agentes:**")
        all_sel = st.checkbox("Seleccionar todos", value=True)
        
        if all_sel:
            sel_agents = ['📊 Todos los agentes']
        else:
            sel_agents = st.multiselect("Seleccionar:", agents, default=[])
            if not sel_agents:
                sel_agents = ['📊 Todos los agentes']
    
    with c2:
        top = st.selectbox("Top:", [5, 10, 20, 50, 100], index=1)
        ...
        with st.expander(f"{icon} **{company}** • {customer} • {fmt(balance)}", expanded=False):
            ...
            <div style='background:#f0f8ff;padding:1rem;border-radius:8px;margin-bottom:1rem'>
                <div style='display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:1rem'>
                    ...
                    <div><small>Days</small><br><b style='color:{S_AMBER}'>{fmt_days(days)}</b></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            with c2:
                if was_touched:
                    ...
                    if days_ago < 9999:
                        st.metric("⏱️ Última", f"{days_ago}d")
                    else:
                        st.metric("⏱️ Última", "?")
                else:
