"""
QA Tab v7.0 - FIXED: Pipeline Robusto
REEMPLAZA: qa_ui_tab_v6.py
"""
...
def render_qa_tab(df_aging, col_payer=None, col_name=None, col_collector=None, col_total=None):
    """Tab Management QA v7.0 - Pipeline robusto con normalización extrema."""
    ...
    try:
        ...
        st.success(f"✅ **{len(acts)} actividades** | **{acts['agent'].nunique()} agentes** | **{touched_count} gestionadas ({touched_pct:.1f}%)** | Matching: {method}")
    ...
    for idx, row in df_view.iterrows():
        ...
        with st.expander(f"{icon} **{company}** • {customer} • {fmt(balance)}", expanded=False):
            st.markdown(f"""
            <div style='background:rgba(0,167,225,0.08);border:1px solid rgba(0,167,225,0.3);padding:1rem;border-radius:8px;margin-bottom:1rem'>
                <div style='display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:1rem'>
                    ...
                    <div><small style='opacity:0.7'>Collector</small><br><b>{collector}</b></div>
            ...
            if was_touched:
                ...
                if match_key and pd.notna(match_key):
                    ...
                    for _, act in history.iterrows():
                        ...
                        st.markdown(f"""
                        <div style='background:rgba(0,167,225,0.05);border-left:3px solid {AMZ_SKY};padding:0.8rem;border-radius:6px;margin:0.5rem 0'>
