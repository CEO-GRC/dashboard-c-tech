"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
QA Tab v3.0 FINAL — Multiselect + Company matching + Real data
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
from qa_module_v3_final import (
...
def render_qa_tab(df_aging):
    """Renderiza el Tab 4 - Management & QA - VERSIÓN FINAL."""
        ...
        <div style='display:flex;align-items:center;gap:1rem'>
            ...
            <div>
                ...
                <p style='color:{AMZ_SKY};margin:0.5rem 0 0 0;font-size:1rem'>
                    Análisis dinámico de cartera y actividades - Cruce por Company Name
                </p>
            </div>
        </div>
    </div>
    ...
    uploaded_file = st.file_uploader(
        "Excel o CSV con: User, Date, Company, History",
        type=['xlsx', 'xls', 'csv'],
    ...
    try:
        ...
        {df_activities['agent'].nunique()} agentes | {df_activities['company_clean'].nunique()} empresas
        """)
        
    except Exception as e:
        st.error(f"❌ Error: {str(e)}")
        st.info("Verifique que el archivo tenga: User, Date, Company")
        return
    ...
    # Filtrar por agentes (multiselect)
    if '📊 Todos los agentes' not in selected_agents and selected_agents:
        ...
        st.info(f"👥 **Filtrando por {len(selected_agents)} agente(s):** {', '.join(selected_agents)}")
    else:
    ...
    # ========================================================================
    # DETALLE POR CUENTA - CON DATOS REALES
    # ========================================================================
    
    st.markdown("### 📋 Detalle por Cuenta")
    st.markdown("*Click para ver historial completo y agregar comentarios*")
    ...
    for idx, row in df_view.iterrows():
        ...
        with st.expander(
            f"{icon} **{company}** • {format_currency(balance)} • {row['activity_status']}",
            expanded=False
        ):
            # Info básica
            st.markdown(f"""
            ...
            if row['was_touched']:
                ...
                # Obtener actividades de esta COMPANY
                company_acts = df_activities[
                    df_activities['company_clean'] == company_clean
                ].sort_values('date', ascending=False)
                
                if len(company_acts) == 0:
                    st.warning("No se encontraron actividades para esta empresa")
                else:
                    for act_idx, act in company_acts.iterrows():
            ...
            comment_key = f"qa_{company_clean}_{idx}"
            existing_comment = st.session_state.qa_comments.get(company, '')
            ...
            new_comment = st.text_area(
                ...
                height=120,
                key=comment_key,
                placeholder="Ej: Cuenta importante sin gestión. Asignar a Diego para seguimiento urgente.",
                label_visibility="collapsed"
            )
            
            if new_comment != existing_comment:
                st.session_state.qa_comments[company] = new_comment
    
    # ========================================================================
    # RESUMEN ÚTIL - KPIs REALES
    # ========================================================================
    ...
    with col1:
        st.metric(
            "Total Cuentas Analizadas",
            total_accounts,
            help="Número de cuentas en el análisis actual"
        )
    ...
    with col2:
        ...
        st.metric(
            "Cuentas Gestionadas",
            touched,
            delta=f"{pct_touched:.1f}%",
            help="Cuentas con al menos una actividad"
        )
    
    with col3:
        st.metric(
            "Balance Total Analizado",
            format_currency(total_balance),
            help="Suma total del balance past due"
        )
    ...
    with col4:
        st.metric(
            ...
            delta="⚠️ Riesgo",
            delta_color="inverse",
            help="Balance de cuentas sin actividad"
        )
    
    # Mostrar agentes más activos
    if len(df_activities) > 0:
        st.markdown("---")
        st.markdown("**👥 Top Agentes por Actividades (en el período analizado):**")
    ...
    with col1:
        if st.button("📊 Generar Excel con Comentarios", 
