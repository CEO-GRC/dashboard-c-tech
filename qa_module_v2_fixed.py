...
def merge_aging_with_activities(df_aging, df_activities, customer_col=None, balance_col=None, collector_col=None):
    """
    Cruza Aging con Activities de forma ROBUSTA y POR CONTENIDO.
    Encuentra automáticamente las columnas con más coincidencias en los datos,
    sin importar el título de la columna.
    """
    aging = df_aging.copy()
    acts = df_activities.copy()
    
    # ========================================================================
    # 1. LA MAGIA: DETECTAR COLUMNAS DE CRUCE POR CONTENIDO (MATCHING)
    # ========================================================================
    best_col_aging = None
    best_col_acts = None
    max_matches = 0
    
    # Escanear todas las columnas de ambos archivos
    for c_aging in aging.columns:
        # Volver set para cruce rápido, limpiando nulos y vacíos
        set_aging = set(aging[c_aging].dropna().astype(str).str.strip().str.lower())
        set_aging.discard('nan')
        set_aging.discard('')
        set_aging.discard('none')
        ...
        for c_acts in acts.columns:
            ...
            set_acts.discard('nan')
            set_acts.discard('')
            set_acts.discard('none')
            ...
            if matches > max_matches:
                max_matches = matches
                best_col_aging = c_aging
                best_col_acts = c_acts
                
    if max_matches == 0:
        raise ValueError("¡Paila! No se encontraron datos en común entre el Aging y las Actividades para hacer el cruce.")
        
    # ========================================================================
    # 2. RENOMBRAR Y ESTANDARIZAR PARA QUE NO SE ROMPA LO DEMÁS
    # ========================================================================
    # Renombramos las columnas ganadoras al estándar "customer_number"
    # Esto asegura que get_agent_portfolio y export_qa_report funcionen sin tocarlos
    if best_col_aging != 'customer_number':
        aging = aging.rename(columns={best_col_aging: 'customer_number'})
    
    if best_col_acts != 'customer_number':
        acts = acts.rename(columns={best_col_acts: 'customer_number'})

    # Limpiamos los datos de cruce
    aging['customer_number'] = aging['customer_number'].astype(str).str.strip()
    acts['customer_number'] = acts['customer_number'].astype(str).str.strip()

    # ========================================================================
    # 3. DETECTAR EL RESTO DE COLUMNAS (Balance, Collector, Company)
    # ========================================================================
    # Balance / Past Due
    if balance_col is None:
        ...
        if balance_col is None:
            ...
            balance_col = numeric_cols[0] if len(numeric_cols) > 0 else aging.columns[1]
    ...
    # Estandarizar Balance y Collector
    ...
    # ========================================================================
    # 4. AGREGAR INFO DE ACTIVIDADES Y HACER EL MERGE
    # ========================================================================
    activities_summary = acts.groupby('customer_number').agg({
    ...
    # Marcar si fue tocada y estatus
